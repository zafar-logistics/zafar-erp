...
ALL_AVAILABLE_COLUMNS = [
    'Company Name', 'Bank Name', 'File No', 'Indenter', 'Supplier Name', import streamlit as st
import pandas as pd
import sqlite3
import json
from datetime import datetime, timedelta
...
def init_db():
    ...
    # Auto-Fix Missing Columns if Table Already Exists
    shipment_cols = [
    ...
    for col_name, col_type in shipment_cols:
        try: c.execute(f"ALTER TABLE shipments ADD COLUMN {col_name} {col_type}")
        except: pass

    item_cols = [
        ('brand_name', 'TEXT'), ('hs_code', 'TEXT'), ('actual_costing', 'TEXT')
    ]
    for col_name, col_type in item_cols:
        try: c.execute(f"ALTER TABLE shipment_items ADD COLUMN {col_name} {col_type}")
        except: pass
