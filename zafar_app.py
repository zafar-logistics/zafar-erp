...
def get_safe_val(row_obj, keys_list, default=""):
    """Case-insensitive bulletproof value retriever for dictionary/pandas row keys"""
    if row_obj is None:
        return default
        
    # Standard dictionary or Series checking
    for k in keys_list:
        if k in row_obj: 
            return row_obj[k]
    ...
    # Advanced Case-Insensitive Check (Safeguard against 'unit' vs 'Unit' vs 'UNIT')
    try:
        available_keys = []
        if hasattr(row_obj, 'keys'):
            available_keys = list(row_obj.keys())
        elif hasattr(row_obj, 'index'):
            available_keys = list(row_obj.index)
            
        lower_keys = [str(ak).lower().strip() for ak in available_keys]
        for k in keys_list:
            lk = str(k).lower().strip()
            if lk in lower_keys:
                idx = lower_keys.index(lk)
                return row_obj[available_keys[idx]]
    except:
        pass
...
# --- SIDEBAR BULK IMPORT ---
if Math_Role := st.session_state["user_role"] in ["Admin", "Manager"]:
    st.sidebar.markdown("---")
