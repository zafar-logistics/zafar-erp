        ...
        try:
            ...
            if 'File No' in df_upload.columns:
                ...
                for index, row in df_upload.iterrows():
                    ...
                    sup = str(row.get('Supplier Name', '-')).strip()
                    tlc = str(row.get('Total LC Value', '-')).strip()
                    curr = str(row.get('Currency', 'USD')).strip()
                    stype = str(row.get('Type', 'FCL')).strip()
                    etd_val = str(row.get('ETD', '-')).strip()
                    eta_val = str(row.get('ETA', '-')).strip()
                    bl_val = str(row.get('BL / LC No', '-')).strip()
                    ...
                    if itm_name != "-":
                        c.execute('''INSERT INTO shipment_items (file_no, item_name, brand_name, hs_code, qty, unit, unit_price, actual_costing) 
                                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                                  (f_no, itm_name, brnd, hsc, qty, unt, u_prc, act_cost))
                    success_count += 1
...
# --- 2. NAYI ENTRY ---
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        shipper = c3.selectbox("Supplier Name (Shipper)", [""] + past_suppliers) if past_suppliers else c3.text_input("Shipper")
        ...
        with st.form(key=f"form_update_{file_to_update}"):
            ...
            for idx in range(3):
                ...
                if idx < len(df_ex_items):
                    ...
                    ex_name, ex_brand, ex_hs, ex_qty, ex_unit, ex_price, ex_cost = item_row['item_name'], item_row['brand_name'], item_row['hs_code'], item_row['qty'], item_row['unit'], item_row['unit_price'], item_row['actual_costing']
            ...
            st.markdown("---")
            u_etd, u_eta, u_bl = u1.text_input("ETD", value=str(row['etd'])), u2.text_input("ETA", value=str(row['eta'])), u1.text_input("BL/LC No", value=str(row['bl_no']))
...
# --- 4. MANAGE ACCOUNTS PANEL ---
    ...
    with m_col1:
        ...
        with st.form("create_user_form"):
            new_user, new_pass = st.text_input("Username:"), st.text_input("Password:", type="password")
    ...
    with m_col2:
        st.write("##### 🔄 Manage Security Profile")
